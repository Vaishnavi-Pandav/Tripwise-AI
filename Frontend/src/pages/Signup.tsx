...
import { Mail, Lock, User, Eye, EyeOff, ArrowLeft, Plane, AlertCircle } from 'lucide-react';
...
const Signup = () => {
  ...
  const strength = password.length === 0 ? 0 : password.length < 6 ? 1 : password.length < 10 ? 2
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(msg.includes('email-already-in-use') ? 'Email already in use' : 'Signup failed. Try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-[#020817] py-12">
      <div className="absolute inset-0 z-0">
        <img src="/bali.png" alt="Background" className="w-full h-full object-cover" />
      ...
      <Link to="/" className="absolute top-8 left-8 z-20 flex items-center gap-2 text-white/70 hover:text-white transition-colors">
        <ArrowLeft size={20} /><span className="font-medium hidden sm:inline">Back to Home</span>
      </Link>
          ...
          <div className="relative z-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 text-white rounded-xl py-2.5 pl-11 pr-4 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 placeholder:text-white/30 text-sm"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 text-white rounded-xl py-2.5 pl-11 pr-12 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 placeholder:text-white/30 text-sm"
                ...
                {password.length > 0 && (
                  <div className="pt-1 px-1">
                    <div className="flex gap-1 mb-1 h-1.5">
                      {[1,2,3,4].map(l => <div key={l} className={`flex-1 rounded-full transition-colors ${strength >= l ? strengthColors[strength] : 'bg-white/10'}`} />)}
                    </div>
              ...
              <button type="submit" disabled={loading}
                className="w-full py-3 mt-4 rounded-xl bg-gradient-to-r from-emerald-500 to-sky-500 text-white font-semibold text-sm transition-all active:scale-[0.98] disabled:opacity-60">
            ...
            <div className="flex items-center gap-4 my-5">
              ...
              <span className="text-[10px] text-white/40 uppercase tracking-wider">Or</span>
              <div className="h-px flex-1 bg-white/10" />
            </div>

            <button type="button" onClick={handleGoogle} disabled={googleLoading}
              className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white text-sm font-medium flex items-center justify-center gap-3 transition-colors disabled:opacity-60">
              {googleLoading
            ...
            <p className="mt-6 text-center text-sm text-white/60">
              ...
              <Link to="/login" className="text-emerald-400 hover:text-emerald-300 font-medium">Log in</Link>
            </p>
          </div>
        </div>
      </motion.div>
